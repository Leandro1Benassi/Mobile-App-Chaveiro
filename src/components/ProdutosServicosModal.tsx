...
import { X, ShoppingCart, Wrench, Users } from "lucide-react";
...
export function ProdutosServicosModal({ isOpen, onClose }: ProdutosServicosModalProps) {
    const { produtos, clientes } = useApp();
    const [servicos, setServicos] = useState<Servico[]>([]);
    const [activeTab, setActiveTab] = useState<"produtos" | "servicos" | "clientes">("produtos");
            ...
            <div className="bg-white w-full rounded-t-lg shadow-lg max-h-[90vh] flex flex-col">
                ...
                <div className="flex gap-0 border-b border-gray-200 px-4 pt-4">
                    ...
                    <button
                        ...
                        className={`pb-3 px-4 font-medium transition-colors flex items-center gap-2 ${activeTab === "clientes"
                            ? "text-indigo-600 border-b-2 border-indigo-600"
                            : "text-gray-600 hover:text-gray-800"
                            }`}
                    >
                ...
                <div className="p-4 border-b border-gray-200">
                    <input
                        ...
                        placeholder={`Buscar ${activeTab === "produtos" ? "produto" : activeTab === "servicos" ? "serviço" : "cliente"}...`}
                            ...
                            {filteredClientes.length === 0 ? (
                                ...
                                filteredClientes.map((cliente) => {
                                    ...
                                    return (
                                        <div
                                            key={cliente.id}
                                            className="bg-gray-50 rounded-lg p-3 border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 transition-colors"
                                        >
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <p className="font-medium text-gray-800">{nome}</p>
                                                    {contato && (
                                                        <p className="text-sm text-gray-500 mt-1">{contato}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
